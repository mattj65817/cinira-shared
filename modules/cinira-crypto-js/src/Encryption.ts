...
async function decryptPrivateKey(data: Buffer, passphrase: string) {
    ...
    if (!bagContents.oids[0].equals(ALGORITHM_IDS.PBKDF2)
    ...
    const passphraseKey = await crypto.subtle.importKey("raw", utf8Encoder.encode(passphrase), "PBKDF2", false, ["deriveBits", "deriveKey"]);
    const decryptionKey = await crypto.subtle.deriveKey({
        ...PBKDF2_ALGORITHM_PARAMS,
        iterations, salt
    }, passphraseKey, AES_CBC_256_ALGORITHM_PARAMS, true, ["decrypt", "encrypt"]);
...
export async function generateAESKey(passphrase: string, salt?: Buffer) {
    ...
    if (null != salt) {
        ...
        saltValue = crypto.createHash("SHA-256")
            .update(encodedPassphrase)
            .digest()
            .subarray(0, 16)
    }
    const passphraseKey = await crypto.subtle.importKey("raw", encodedPassphrase, PBKDF2_ALGORITHM_PARAMS, false, ["deriveBits", "deriveKey"]);
